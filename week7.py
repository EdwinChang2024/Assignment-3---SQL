...
def question2():
    # write your query below
	query = "SELECT DISTINCT DEPARTMENT FROM Worker ORDER BY DEPARTMENT ASC;"
	df = pd.read_sql(query, connection)
...
def question3():
    # write your query below
	query = "SELECT FIRST_NAME, SALARY FROM Worker ORDER BY FIRST_NAME ASC, SALARY DESC;"
	df = pd.read_sql(query, connection)
...
def question4():
    # write your query below
	query = "SELECT DEPARTMENT, COUNT(WORKER_ID) AS WORKER_NUM FROM Worker GROUP BY DEPARTMENT ORDER BY COUNT(WORKER_ID) ASC;"
	df = pd.read_sql(query, connection)
...
def question5():
    # write your query below
	query = "SELECT Title.WORKER_TITLE, Worker.FIRST_NAME, Worker.SALARY FROM Worker INNER JOIN Title on Worker.WORKER_ID = Title.WORKER_REF_ID ORDER BY Worker.SALARY DESC, Worker.FIRST_NAME ASC;"
	df = pd.read_sql(query, connection)
...
def question6():
    # write your query below
	query = "SELECT t.WORKER_TITLE, w.FIRST_NAME, w.SALARY FROM Worker w INNER JOIN Title t ON w.WORKER_ID = t.WORKER_REF_ID WHERE w.SALARY IN (SELECT SALARY FROM Worker GROUP BY SALARY HAVING COUNT(*) > 1) ORDER BY w.SALARY DESC, w.FIRST_NAME ASC;"
	df = pd.read_sql(query, connection)
...
def question7():
    # write your query below
	query = "SELECT DEPARTMENT, COUNT(WORKER_ID) AS No_Of_Workers FROM Worker GROUP BY DEPARTMENT HAVING COUNT(WORKER_ID) = 4;"
	df = pd.read_sql(query, connection)
